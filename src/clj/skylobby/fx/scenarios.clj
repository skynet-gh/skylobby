...
(defn scenarios-window-impl
  [{:fx/keys [context]
    :keys [screen-bounds]}]
  (let [show-scenarios-window (fx/sub-val context :show-scenarios-window)]
    {:fx/type :stage
     :showing (boolean show-scenarios-window)
      ...
      (if show-scenarios-window
        (let [
              ...
              rapid-tasks-by-id (->> (fx/sub-ctx context skylobby.fx/tasks-of-type-sub :spring-lobby/rapid-download)
                                     ...
                                     (into {}))
              mod-update-tasks-by-id (->> (fx/sub-ctx context skylobby.fx/tasks-of-type-sub :spring-lobby/rapid-download)
                                          (map (juxt :rapid-id identity))
                                          (into {}))]
          {:fx/type :v-box
